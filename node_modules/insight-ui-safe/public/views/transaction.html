        ...
        <div data-ng-if="tx.txid">
            ...
            <table class="table" style="table-layout: fixed">
                ...
                <tr data-ng-show="tx.fees">
                    ...
                    <!--<td class="text-muted text-right">{{$root.currency.getConvertion((tx.fees * 1000) / tx.size) + ' per kB' || ((tx.fees * 1000) / tx.size) + 'SAFE per kB'}}-->
                    <td class="text-muted text-right">
                        {{ FormatFeeRate((tx.fees * 1000) / tx.size) + ' SAFE per kB'}}
                    </td>
                </tr>
                <tr>
                    <td><strong translate>Received Time</strong></td>
                    <td data-ng-show="tx.time" class="text-muted text-right">{{tx.time * 1000|date:'yyyy-MM-dd HH:mm:ss'}}
                    </td>
                    <td data-ng-show="!tx.time" class="text-muted text-right">N/A</td>
                </tr>
                <tr>
                    <td><strong translate>Mined Time</strong></td>
                    <td data-ng-show="tx.blocktime" class="text-muted text-right">{{tx.blocktime * 1000|date:'yyyy-MM-dd HH:mm:ss'}}
                    </td>
