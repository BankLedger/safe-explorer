...
/**
 * 从数据库中获取是否存在与当前高度范围内解锁的钱包地址
 * @param {*} height 
 * @param {*} callback 
 */
 MysqlUtils.prototype.getUnlockAddressRangeHeight = function( heightStart , heightEnd , callback ){
    var sql = "select DISTINCT address from lockedAddresses where unlockedHeight >= ? and unlockedHeight <= ?";
    var option = [heightStart,heightEnd];
    this.sql( sql, option, function( err , result ){
        if(err) console.log( "获取指定高度解锁交易钱包地址 [:lockedaddresses] 错误 : " +  JSON.stringify(err));
        callback( null , result );
    } )
