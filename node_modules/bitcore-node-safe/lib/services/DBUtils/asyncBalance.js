    ...
    async.whilst(
        ...
        function(callback){         // Execution
            ...
            self.handleBlock( dbHeight , function( err ){
                ...
                // 当完成一个区块内的地址余额更新时,更新表,及self.process{ dbHeight }
                if ( self.process.dbHeight + 1 == self.process.latestHeight || (self.process.dbHeight + 1) % StepRange == 0 ){
                    // 想了一下,不要每个区块统计完都再统计更新一次,跑到最新高度,或者每隔 1000个区块高度的时候,更新一次.
                    self.handleHeightStatisticUpdate( dbHeight , function(err , result){
                        if (err) return callback(err);
                        self.process.dbHeight ++ ;
                        self.log("finish height(with handle_HeightStatisticUpdate):" + dbHeight)
                        callback(); 
                    });
                }else{
                    self.log("finish height:" + dbHeight)
                    self.process.dbHeight ++ ;
                    callback(); 
                }
            } );
...
AsyncBalanceUpdate.prototype.handleBlock = function( height , callback ){
    ...
    async.waterfall( [
        ...
        function ( addrArr , callback ){    // 如果存在当前高度解锁了的地址，则需要将这些地址查询出来，再下一步的函数中进行余额更新
            if ( height == self.process.latestHeight ){
                self.log("查询当前高度的解锁地址 , height =" + height)
                self.DB.getUnlockAddress( height , function( err , result ){
                    if ( err ) return callback( err , result );
                    self.log("当前高度的解锁地址数量" + result.length)
                    result.forEach( element => addrArr.push( element.address ) );
                    callback( null , addrArr );  
                });
            } else if ( height % StepRange == 0 ){
                self.log("查询当前高度范围内的解锁地址:" + ( height- StepRange + "::" ) + (height) )
                self.DB.getUnlockAddressRangeHeight( height - StepRange , height , function( err , result ){
                    if ( err ) return callback( err , result );
                    self.log("当前高度范围内的解锁地址数量:" + result.length)
                    result.forEach( element => addrArr.push( element.address ) );
                    callback( null , addrArr );  
                });
            }else {
                self.log("略过当前高度的解锁地址更新")
                callback( null , addrArr );  
            }
        },
