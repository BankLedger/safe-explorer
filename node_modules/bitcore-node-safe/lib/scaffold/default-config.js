...
/**
 * Will return the path and default bitcore-node-safe configuration. It will search for the
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.safecore');
  var defaultConfigFile = path.resolve(defaultPath, './bitcore-node-safe.json');
  ...
  if (!fs.existsSync(defaultConfigFile)) {
    var defaultConfig = {
      ...
      servicesConfig: {
        bitcoind: {
          spawn: {
            datadir: path.resolve(process.env.HOME, './.safe_data'),
            exec: path.resolve(__dirname, process.env.HOME, 'safe/safed')
          }
        }
      }
    };
