...
To develop bitcore-node-safe:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-safe.git
...
We now will setup symlinks in `bitcore-node-safe` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-safe
ln -s ~/bitcoind-rpc-safe
```
...
cd devnode
mkdir node_modules
touch bitcore-node-safe.json
touch package.json
```

Edit `bitcore-node-safe.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-safe/bin/bitcore-node-safe start
```
